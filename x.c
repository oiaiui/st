...
static int focused = 0; // alpha+focus addition
...
// xloadcols replacement provided by the alpha+focus patch
 void
 xloadcols(void)
 {
 	static int loaded;
...
xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y, int dmode)
{
	...
	if (dmode & DRAW_BG) {
		...
		if (x == 0) {
        	xclear(0, (y == 0)? 0 : winy, borderpx_x, winy + win.ch + ((winy + win.ch >= borderpx_y + win.th)? win.h : 0)); // dont touch
		}
	    if (winx + width >= borderpx_x + win.tw) { // dont touch
			xclear(winx + width, (y == 0)? 0 : winy, win.w, ((winy + win.ch >= borderpx_y + win.th)? win.h : (winy + win.ch)));
		}
		if (y == 0)
        	xclear(winx, 0, winx + width, borderpx_y); // dont touch
        if (winy + win.ch >= borderpx_y + win.th)
			xclear(winx, winy + win.ch, winx + width, win.h); // please dont touch
...
{
	XPoint spot = { borderpx_x + x * win.cw, borderpx_y + (y + 1) * win.ch };
...
xrdb_load(void)
{
	...
	if (xrm != NULL) {
		...
		XRESOURCE_LOAD_INTEGER("borderpx_x", borderpx_x);
		XRESOURCE_LOAD_INTEGER("borderpx_y", borderpx_y);
